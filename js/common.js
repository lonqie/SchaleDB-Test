...
function loadItem(id) {
    if (loadedModule == 'items') {
        ...
        $('#ba-item-icon-img').attr('src', `images/${mode}/icon/${item.Icon}.webp`)
...
function getDropIconHTML(id, chance, qtyMin=1, qtyMax=1, forcePercent=false, dropType=null) {
    ...
    if (type == "GachaGroup") {
        ...
        html = `<div class="item-drop drop-shadow" style="position: relative; ${haslink ? 'cursor:pointer;" onclick="loadItem('+id+')"' : '"'} title="${getRichTooltip(`images/${iconPath}/icon/${item.Icon}.webp`, getTranslatedString(item, 'Name'), getLocalizedString('ItemCategory',itemType), rarityText, description, 50, 'img-scale-larger')}"><img class="ba-item-icon ba-item-${item.Rarity.toLowerCase()}" src="images/${iconPath}/icon/${item.Icon}.webp"><span class="ba-material-label" ${haslink ? 'style="cursor:pointer;"' : ""}>${qtyMax > 1 || forcePercent ? parseFloat((chance*100).toFixed(2)) + '&#37;' : getProbabilityText(chance)}</span>${qtyMax > 1 ? `<span class="label-qty">&times;${qtyMin != qtyMax ? abbreviateNumber(qtyMin) + '~' + abbreviateNumber(qtyMax) : abbreviateNumber(qtyMax)}</span>` : ''}${dropType != null ?  `<span class="label-droptype">${dropType}</span>` : ''}</div>`
    }
    return html
}
        ...
        interactiveStudents.forEach(interaction => {
            if (interactiveStudents.length == 1) {
                ...
                if (interaction[1] == 1 || (interaction[1] == 0 && interactiveStudents.some(i => i[1] == 2))) {
                    html += getStudentIconSmall(interaction[0], '<i class="fa-solid fa-user"></i><span class="mx-1">/</span><i class="fa-solid fa-user-group"></i>')
                } else if (interaction[1] == 0) {
                    html += getStudentIconSmall(interaction[0], '<i class="fa-solid fa-user-group"></i>')
                } else if (interaction[1] == 2) {
                    html += getStudentIconSmall(interaction[0], '<i class="fa-solid fa-user-plus"></i>')
